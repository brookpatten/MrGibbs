	...
	public class TSAGeoMag
    {
        ...
        private void initModel()
        {
            ...
            catch (Exception ex)
            {
                ...
                writer.Write(string.Join(System.Environment.NewLine, input));
                writer.Flush();
                stream.Position = 0;
        ...
        private void ReadCOFStream(StreamReader sr)
        {
            ...
            while (true)
            {
                ...
                if (line[0].StartsWith("9999")) // end of file
                    break;

                int n = int.Parse(line[0]);
                int m = int.Parse(line[1]);
                double gnm = double.Parse(line[2]);
                double hnm = double.Parse(line[3]);
                double dgnm = double.Parse(line[4]);
                double dhnm = double.Parse(line[5]);
        ...
        //    epoch = Double.Parse(input[0].Trim().Split("[\\s]+")[0]);
        //    defaultDate = epoch + 2.5;
        ...
        //        double gnm = Double.Parse(tokens[2]);
        //        double hnm = Double.Parse(tokens[3]);
        //        double dgnm = Double.Parse(tokens[4]);
        //        double dhnm = Double.Parse(tokens[5]);
        ...
        public static double decimalYear(DateTime date)
        {
            ...
            var next = new DateTime(date.Year + 1, 1, 1, 0, 0, 0);
            var lastDayOfThisYear = next.Subtract(new TimeSpan(1, 0, 0));
            var daysInYear = lastDayOfThisYear.DayOfYear;

            return (double)year + ((double)dayOfYear / (double)daysInYear);
